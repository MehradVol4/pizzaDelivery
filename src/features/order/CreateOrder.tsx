...
import type { CartItem, NewOrder } from "../../types";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const cart = JSON.parse(String(data.cart ?? "[]")) as CartItem[];
  const order: NewOrder = {
    customer: String(data.customer ?? ""),
    phone: String(data.phone ?? ""),
    address: String(data.address ?? ""),
    cart,
    priority: String(data.priority ?? "false") === "true",
    position: String(data.position ?? "") || undefined,
  };
